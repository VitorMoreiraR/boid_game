#include "primitives.hpp"
    ...
    vec3
    Primitives::cross(const vec3 &a, const vec3 &b) {
...
vec3 Primitives::normalize(const vec3 &v) {
...
void Primitives::rect(vec3 p1, vec3 p2, vec3 p3, vec3 p4, color cor) {
...
void Primitives::desenhaCubo(float SIZE, Colors colors) {
  float d = SIZE / 2.0;
  ...
  rect(v1, v2, v3, v4, colors.vermelho);

  // direita
  glNormal3f(1.f, 0.f, 0.f);
  rect(v4, v3, v6, v5, colors.azul);

  // back
  glNormal3f(0.f, 0.f, -1.f);
  rect(v5, v6, v7, v8, colors.amarelo);

  // esquerda
  glNormal3f(-1.f, 0.f, 0.f);
  rect(v1, v8, v7, v2, colors.verde);

  // topo
  glNormal3f(0.f, 1.f, 0.f);
  rect(v1, v4, v5, v8, colors.violeta);

  // bottom
  glNormal3f(0.f, -1.f, 0.f);
  rect(v2, v7, v6, v3, colors.laranja);
}

void Primitives::desenhaEsfera(GLfloat raio, GLuint nStacks, GLuint nSectors,
                               Colors colors) {
...
void Primitives::desenhaCone(GLfloat altura, GLfloat raioBase, GLuint nStacks,
  ...
  for (GLuint i = 0; i < nStacks; i++) {
    if (i % 3 == 0)
      ...
      glColor3fv(colors.cinza);
    else
      glColor3fv(colors.azul);
...
void Primitives::desenharPiramide(float baseComprimento, float baseLargura,
                                  float altura, Colors colors) {
  glPushMatrix();
...
void Primitives::desenhaParalelepipedo(float largura, float altura,
                                       float profundidade, Colors colors) {
  ...
  // Traseira
  rect(v5, v8, v7, v6, colors.amarelo);

  // Esquerda
  rect(v1, v8, v7, v2, colors.verde);

  // Topo
  rect(v1, v4, v5, v8, colors.laranja);

  // Base
  rect(v2, v7, v6, v3, colors.vermelho);
}
void Primitives::desenhaTerreno(unsigned int id, Colors colors) {
  float L = 500.0;
  float y = -0.5f;
